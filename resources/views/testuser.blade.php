        ...
        <div class="col-12">
            <div class="container">
                <div class="role-perms mt-5">
                    <h4>Role: {{ $role->name }} to <span class="text-success">{{ $user->name }}</span></h4>
                    <form action="{{ route('testuserPost') }}" method="post">@csrf
                    @foreach ($permissions as $perm)
                    <h5 class="mt-5">{{ $perm->name }}</h5>
                    @if(!empty($perm->permissions))
                        @foreach ($perm->permissions as $sub_perm)
                        @if($role->hasPermission($sub_perm->id))
                        <div class="form-check form-check-inline">
                            <input type="hidden" class="perms_unchecked" name="perms_unchecked[]" data-unchecked-id="{{ $sub_perm->id }}" value="">
                            <input class="form-check-input check-demo" data-id="{{ $sub_perm->id }}" @if($user->hasPermission($sub_perm->id)) checked @endif name="perms[]" type="checkbox" id="{{ $sub_perm->slug }}" value="{{ $sub_perm->id }}">
